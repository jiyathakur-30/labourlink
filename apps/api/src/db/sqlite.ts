...
// pure JS persistence file database to avoid Windows node-gyp compile issues with native modules!
const dbDir = path.join(__dirname, '../../../../database');
if (!fs.existsSync(dbDir)) {
  fs.mkdirSync(dbDir, { recursive: true });
}
const dbPath = path.join(dbDir, 'labourlink_db.json');
    ...
    if (sqlLower.includes('from bookings b') && sqlLower.includes('left join payments p')) {
      ...
      if (filterUserId) {
        list = data.bookings.filter(b => {
          if (sqlLower.includes('wp.id = ?') || sqlLower.includes('wp.user_id = ?')) {
      ...
      const results = list.map(b => {
        const wp = data.worker_profiles.find(w => w.id === b.worker_id);
        const u = wp ? data.users.find(usr => usr.id === wp.user_id) : null;
